...
function TranscriptProgramIndex() {
  ...
  const getData = async () => {
    await axios
      .get(
        `${ApiUrl}/academic/fetchAllProgramTranscriptDetails?page=${0}&page_size=${100}&sort=created_date`
      )
      .then((Response) => {
  ...
  const handleActive = async (params) => {
    ...
    const handleToggle = () => {
      if (params.row.active === true) {
        axios
          .delete(`${ApiUrl}/academic/ProgramTranscriptDetails/${id}`)
          .then((res) => {
            if (res.status == 200) {
              getData();
            }
        ...
        axios
          .delete(`${ApiUrl}/academic/activateProgramTranscriptDetails/${id}`)
          .then((res) => {
            if (res.status == 200) {
              getData();
            }
