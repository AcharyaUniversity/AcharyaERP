...
function TranscriptIndex() {
  ...
  const getData = async () => {
    await axios
      .get(
        `${ApiUrl}/academic/fetchAllProgramTranscript?page=${0}&page_size=${100}&sort=created_date`
      )
      .then((Response) => {
  ...
  const handleActive = (params) => {
    ...
    const handleToggle = () => {
      if (params.row.active === true) {
        axios
          .delete(`${ApiUrl}/academic/deactivateProgramTranscript/${id}`)
          .then((res) => {
            if (res.status == 200) {
              getData();
              setModalOpen(false);
        ...
        axios
          .delete(`${ApiUrl}/academic/activateProgramTranscript/${id}`)
          .then((res) => {
            if (res.status == 200) {
              getData();
              setModalOpen(false);
  ...
  const columns = [
...

    {
      ...
      headerName: "Update",
      getActions: (params) => [
        <Link to={`/TranscriptMaster/Transcript/Update/${params.row.id}`}>
