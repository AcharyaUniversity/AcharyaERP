  ...
  const handleActive = (params) => {
    ...
    const handleToggle = () => {
      if (params.row.active === true) {
        axios.delete(`${ApiUrl}/finance/Ledger/${id}`).then((res) => {
          if (res.status == 200) {
            getData();
            setModalOpen(false);
          }
        ...
        axios.delete(`${ApiUrl}/finance/ActivateLedger/${id}`).then((res) => {
          if (res.status == 200) {
            getData();
            setModalOpen(false);
          }
  ...
  const columns = [
    ...
    },
    {
      ...
      getActions: (params) => [
        <IconButton
          onClick={() =>
            navigate(`/AccountMaster/Ledger/Update/${params.row.id}`)
          }
        >
          <EditIcon />
        </IconButton>,
