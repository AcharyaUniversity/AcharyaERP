...
function RoomForm() {
  ...
  const getRoomsData = async () => {
    await axios
      ...
      .then((res) => {
        ...
        axios
          .get(`${ApiUrl}/getFloors/${res.data.data.block_id}`)
          .then((res) => {
            setFloorName(
  ...
  const handleChangeAdvance = (name, newValue) => {
    if (name === "blockId") {
      axios.get(`${ApiUrl}/getFloors/${newValue}`).then((res) => {
        setFloorName(
          res.data.data.map((object) => ({
            value: object.floor_id,
            label: object.floor_name,
