...
function VoucherAssignmentForm() {
  ...
  const getData = async () => {
    await axios
      ...
      .then((res) => {
        ...
        setVoucherId(data.voucher_head_id);
        setCrumbs([
          { name: "AccountMaster", link: "/AccountMaster" },
          { name: "Voucher Assignment" },
          { name: "Update" },
  ...
  const handleChangeAdvance = (name, newValue) => {
    if (name === "voucherId") {
      axios
        .get(`${ApiUrl}/finance/allUnassignedSchoolDetails/${newValue}`)
        .then((res) => {
          setSchoolOptions(
            res.data.data.map((obj) => ({
              value: obj.school_id,
    ...
    if (Object.values(formValid).includes(false)) {
      ...
      await axios
        ...
        .then((res) => {
          ...
          setAlertOpen(true);
          navigate("/VoucherAssignmentIndex", { replace: true });
        })
        .catch((error) => {
          setLoading(false);
          setAlertMessage({
  ...
  const handleUpdate = async () => {
    if (Object.values(formValid).includes(false)) {
      ...
      temp.voucher_priority = values.voucherPriority;
      temp.school_id = values.school;
      temp.voucher_head_id = values.voucherHeadId;

      await axios
        .put(`${ApiUrl}/finance/VoucherHead/${voucherId}`, temp)
        .then((res) => {
          setLoading(true);
